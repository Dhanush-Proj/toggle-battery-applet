...
use cosmic::iced::{window::Id, Limits};
...
impl cosmic::Application for AppModel {
    ...
    fn init(core: cosmic::Core, _flags: Self::Flags) -> (Self, Task<cosmic::Action<Self::Message>>) {
        let initial_conservation = fs::read_to_string(BAT_PATH)
            .map(|s| s.trim() == "1")
            .unwrap_or(false);

        let initial_percent = fs::read_to_string(SHOW_PERCENT_PATH)
            .map(|s| s.trim() == "true")
            .unwrap_or(false);
        ...
        let app = AppModel {
            core,
            popup: None,
            conservation_enabled: initial_conservation,
            show_percentage: initial_percent,
        };
    ...
    fn view_window(&self, _id: Id) -> Element<'_, Self::Message> {
        let content = widget::list_column()
            ...
            .add(widget::settings::item(
                "Show Percentage",
                widget::toggler(self.show_percentage)
                    .on_toggle(Message::TogglePercentage),
            ));
        ...
        match message {
            Message::TogglePopup => {
                if let Some(p) = self.popup.take() {
                    ...
                    settings.positioner.size_limits = Limits::NONE.min_width(300.0).min_height(120.0);
            ...
            Message::TogglePercentage(enabled) => {
                self.show_percentage = enabled;
                let val = if enabled { "true" } else { "false" };
                // Using fs::write for user-owned config files
                let _ = fs::write(SHOW_PERCENT_PATH, val);
            }
        }
        Task::none()
    }
